...
function addLog(book_id, operation, time) {
    let link = 'http://localhost:8113/operations';
    axios.post(link, {
        'id_book': book_id,
        'name': operation,
        'time': time
...
function addBookToFavorite(e) {
    console.log(this)
    let imageUrl = document.querySelector('.img_book img') ? document.querySelector('.img_book img').src : null;
    let bookTitle = document.querySelector('.title_book') ? document.querySelector('.title_book').innerHTML : null;
    let bookId = this.dataset.id
    axios.get('http://localhost:8113/favorites?bookId=' + bookId)
        .then(function (response) {
            if (response.data === undefined || response.data.length == 0) {
...
function showOnFavPage() {
    ...
    axios.get('http://localhost:8113/favorites').then(function (response) {
        ...
        let btns = allFavorites.querySelectorAll('.add_to_favorites'); // Select all matching buttons
        console.log(btns);
        btns.forEach(btn => {
            btn.addEventListener('click', addBookToFavorite);
        });
    });
}
...
function edit_book() {
    ...
    axios.get('http://localhost:8113/items/' + bookId).then(function (response) {
        ...
        let imageJson = book.volumeInfo.imageLinks ? book.volumeInfo.imageLinks.thumbnail : null;
        let autorsJson = book.volumeInfo.authors ? book.volumeInfo.authors.join(', ') : null;
        let date = new Date(book.volumeInfo.publishedDate);
        console.log(titleJson);
        let year_bookJson = date.getFullYear();
        let desc_bookJson = book.volumeInfo.description ? book.volumeInfo.description : null;
        let ISBNJson = book.volumeInfo.industryIdentifiers ? book.volumeInfo.industryIdentifiers[0].identifier : null;
        let book_page_countJSON = book.volumeInfo.pageCount ? book.volumeInfo.pageCount : null;
        let book_publisherJSON = book.volumeInfo.publisher ? book.volumeInfo.publisher : null;
        let book_languageJSON = book.volumeInfo.language ? book.volumeInfo.language : null;
...
function getHistory() {
    ...
    axios.get('http://localhost:8113/operations').then(function (response) {
